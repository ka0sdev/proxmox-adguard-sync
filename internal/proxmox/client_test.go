...
func TestGetQEMUConfig(t *testing.T) {
	server := httptest.NewServer(
		http.HandlerFunc(func(
			...
			expectedPath := "/api2/json/nodes/pm/qemu/101/config"
			...
			_, _ = writer.Write([]byte(`{
				"data": {
					"name": "devbox-vm",
					"description": "dns_ip=172.20.20.10\ndns_name=devbox",
					"ipconfig0": "ip=172.20.20.10/24,gw=172.20.20.1"
				}
			}`))
		}),
	)
	...
	config, err := client.GetQEMUConfig(
		context.Background(),
		"pm",
		101,
	)
	if err != nil {
		t.Fatalf(
			"GetQEMUConfig() returned an unexpected error: %v",
			err,
		)
	}

	expectedDescription := "dns_ip=172.20.20.10\ndns_name=devbox"

	if actual := config.StringValue(
		"description",
	); actual != expectedDescription {
		t.Errorf(
...
func TestGetQEMUConfigRejectsInvalidIdentity(t *testing.T) {
	client, err := NewClient(ClientOptions{
		BaseURL:     "https://proxmox.example.com/api2/json",
		TokenID:     "dns-sync@pve!adguard-sync",
		TokenSecret: "test-secret",
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if err == nil {
				t.Fatal(
					"GetQEMUConfig() returned nil error",
				)
			}
